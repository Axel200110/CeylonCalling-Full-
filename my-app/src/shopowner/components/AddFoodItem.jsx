...
const AddFoodItem = ({ onClose, onAdd }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (food.discountPrice) formData.append("discountPrice", food.discountPrice);
      if (food.description) formData.append("description", food.description);
      formData.append("availability", food.availability || "available");
      formData.append("tag", food.tag || "standard");
      if (food.picture) {
      ...
      >
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4 relative z-10">
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-semibold text-slate-300 uppercase tracking-wider flex items-center gap-1.5">
                <CheckCircle2 className="w-3 h-3 text-emerald-400" /> Initial Stock Status
              </label>
              <select
                name="availability"
